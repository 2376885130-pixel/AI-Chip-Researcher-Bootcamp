...
//==========================================================
// Day 18 : Systolic 2x2 Matrix Multiply - Testbench
//==========================================================
//
// Test 1: A=[1 2;3 4]  B=[5 6;7 8]   -> C=[19 22;43 50]
// Test 2: A=[-1 2;3 -4] B=[2 -1;0 3] -> C=[-2 7;6 -15]
//
// Compares latency with Day16 (serial ~88) and
// Day17 (parallel 2 cycles).
//==========================================================

module systolic_matmul_2x2_tb;
    ...
    //--------------------------------------------------
    // DUT : systolic array (uses pe_unit from Day14)
    //--------------------------------------------------
    ...
    initial begin
        ...
        $dumpfile("Simulation/Day18/systolic_matmul_2x2.vcd");
        $dumpvars(0, systolic_matmul_2x2_tb);
        ...
        $display("--------------------------------");
        $display("DAY18 SYSTOLIC MATMUL TEST START");
        $display("--------------------------------");

        //--------------------------------------------------
        // Test 1 : same data as Day16/17
        ...
        $display("");
        $display("--------------------------------");
        $display("LATENCY COMPARISON (2x2 matrix)");
        $display("  Day16 serial    : ~88 cycles");
        $display("  Day17 parallel  :  2 cycles");
        $display("  Day18 systolic  : %0d cycles (test1)", latency1);
        $display("  Day18 systolic  : %0d cycles (test2)", latency2);
        $display("");
        $display("  Note: systolic is slightly slower for a tiny");
        $display("  2x2 case due to pipeline fill + drain, but it");
        $display("  wins at scale through data reuse (lower I/O).");

        $display("");
        if (fail_count == 0) begin

            $display("###############################");
            $display("#  DAY18 SYSTOLIC MATMUL PASS #");
            $display("###############################");
            ...
            $display("###############################");
            $display("#  DAY18 SYSTOLIC MATMUL FAIL #");
            $display("###############################");
