...
//==========================================================
// Day 22 : npu_storepipe_top - Pipelined Store Testbench
//==========================================================
// Loads NUM_TASKS (4) matrix pairs packed into 32-bit words
// (4 elements per word), runs the pipelined NPU with a
// decoupled store, and verifies all results against a software
// reference model.
// Tasks (same as Day20/21): identity, [1..16]x[16..1],
// scaled, negatives.
//==========================================================

module npu_storepipe_tb;
    ...
    initial begin
        ...
        $dumpfile("Simulation/Day22/npu_storepipe.vcd");
        $dumpvars(0, npu_storepipe_tb);
        ...
        $display("--------------------------------");
        $display("DAY22 STORE PIPE TEST START");
        $display("--------------------------------");
        ...
        $display("  Day22 pipelined store  : %0d cycles", total_cycles);

        $display("");
        if (total_fail == 0) begin

            $display("###############################");
            $display("#   DAY22 STORE PIPE PASS     #");
            $display("###############################");
