...
//==========================================================
// Day 22 : npu_storepipe_top (pipelined store / output ping-pong)
//==========================================================
//
// Day21 wide-fetch NPU + decoupled store. When a task's compute
// finishes, its 16 results are latched in ONE cycle into an
// output bank; the systolic immediately starts the next task,
// while a concurrent store sub-FSM writes the latched results
// to the output buffer over 16 cycles.
//
//   Day21: compute -> store -> compute -> store   (store serial)
//   Day22: compute -> [latch 1 cyc] -> compute    (store runs
//                        |                            in parallel)
//                      store to output buffer
//
// Two output banks (ping-pong) hold results while the single
// result-memory write port serializes the stores.
//==========================================================

module npu_storepipe_top #(
    ...
    assign store_waddr = store_task*BLOCK + store_cnt;
    ...
    output_buffer #(
        ...
        .write_data(out_bank[store_task[0]][store_cnt]),
